...
<div id="wrapper">
	...
	<div id="content">
			...
			else
			{
				...
				{
					$q="select event_id from a_teamname where teamname='".$teamname."'";
					$r=mysql_query($q);
					if(mysql_num_rows($r))
					{
						if($row=mysql_fetch_assoc($r))
							$event_id=$row['event_id'];
						$newfile=$event_id.'/'.$teamname.'_'.date('d-m-Y-His').'_'.$filename;
						$success=move_uploaded_file($temp,$folder.$newfile);
						$qr="insert into a_files values('".$user->data['user_id']."','".$event_id."','".$teamname."','".$newfile."')";
						if($success){
							mysql_query($qr);
							echo "<p>`$filename` File Uploaded Successfully.</p>";
						}
						else
							echo "<p>File upload failed, please try again.</p>";
					}
					else 
						echo "<p>Event ID not found.</p>";
				}
				else
					echo "<p>Only ZIP and RAR files can be uploaded.</p>";
			}	
		?>
